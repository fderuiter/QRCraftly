...
describe('ESLint Sandbox Isolation', () => {
  ...
  it('should block imports referencing the experiments directory from a production file', async () => {
    const eslint = new ESLint();
    const results = await eslint.lintText(
      "import ExpPrototype from '../../../experiments/prototype/Component';\nconsole.log(ExpPrototype);\n",
      { filePath: join(process.cwd(), 'src/pages/about/+Page.tsx') }
    );
    ...
    expect(restrictedImportErrors.length).toBeGreaterThan(0);
    expect(restrictedImportErrors[0].message).toContain(
      'Experimental workspace assets cannot be imported into production modules.'
    );
  });

  it('should allow imports referencing other modules from inside the experiments directory', async () => {
    const eslint = new ESLint();
    const results = await eslint.lintText(
      "import { TextField } from '../src/components/ui/FormFields';\nconsole.log(TextField);\n",
      { filePath: join(process.cwd(), 'experiments/prototype/Component.tsx') }
    );

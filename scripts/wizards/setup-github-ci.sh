...
banner "GitHub Actions CI/CD Secrets Setup"

# ── Stage 1: GitHub CLI Authentication ──────────────────────────────────────
stage "GitHub: CLI Authentication"
say "We verify your GitHub CLI authentication and repository access."
if command -v gh >/dev/null 2>&1; then
  step "Checking GitHub CLI authentication..."
  if gh auth status >/dev/null 2>&1; then
    say "✓ GitHub CLI authenticated successfully."
  else
    warn "gh CLI is installed but not authenticated."
    step "Run 'gh auth login' in a separate terminal, then continue."
  fi
else

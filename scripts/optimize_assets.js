...
async function optimizeImage(filepath, size = { width: 192, height: 192 }) {
  ...
  try {
    ...
    console.log(
      `Original size of ${filepath}: ${(stats.size / 1024).toFixed(2)} KB`,
    );
    ...
    console.log(
      `New size of ${filepath}: ${(newStats.size / 1024).toFixed(2)} KB`,
    );

    const newMetadata = await sharp(filepath).metadata();
    console.log(
      `New dimensions: (${newMetadata.width}, ${newMetadata.height})`,
    );
  } catch (e) {
...
async function main() {
  ...
  if (fs.existsSync("star_style.png")) {
    try {
      ...
      console.log(
        `star_style.png dimensions: (${metadata.width}, ${metadata.height})`,
      );

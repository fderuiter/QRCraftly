...
- **Scannability Web Workers & Double-Buffered WASM Pipeline**: We utilize Web Workers (`scannabilityWorker.ts`) integrated with an off-thread WebAssembly (WASM) decoding engine and a pre-allocated double-buffered memory pool (`DoubleBufferPool`) to perform real-time QR code scannability testing on a background thread. This double-buffered pipeline recycles pre-allocated `ArrayBuffer` instances between the main thread and background worker thread via zero-copy transferrable objects in worker postMessage calls, eliminating runtime garbage collection pauses. Upon initialization, the worker instantiates an off-thread WebAssembly module to execute two-pass orientation and polarity analysis. If WebAssembly instantiation fails or is unsupported, the system gracefully falls back to JavaScript decoding (`jsQR`). The adaptive scheduler dynamically throttles scan execution frequency when latency approaches 100ms, and cooperative yielding ensures stale scan requests are aborted immediately when new board updates occur. In environments where Web Workers are blocked or unsupported, a try-catch guarded initialization automatically falls back to synchronous main-thread processing, employing similar scheduling strategies (`requestIdleCallback` or `setTimeout`) to maintain a highly responsive UI without frame stutter.

...
- **Scannability Web Workers & Main-Thread Fallback**: We utilize Web Workers (`scannabilityWorker.ts`) to perform real-time QR code scannability testing on a background thread. This ensures the main UI thread remains responsive and jank-free even during rapid configuration changes. To prevent background processing backlog and excessive battery drain during rapid slider adjustments, the worker implements a cooperative cancellation system. By converting the message handler into an asynchronous execution loop with yielding points (via `setTimeout`), the worker can receive newer incoming message configurations, identify older obsolete tasks through tracked sequence IDs, and immediately abort stale runs without incurring thread recreation overhead. In environments where Web Workers are blocked, unsupported, or fail to initialize, a try-catch guarded initialization automatically falls back to synchronous main-thread processing (`scannabilityChecker.ts`), employing similar scheduling strategies (`requestIdleCallback` or `setTimeout`) to maintain a highly responsive, scrollable UI under 200ms lag.

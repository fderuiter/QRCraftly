...
export class MockD1Database implements D1Database {
  ...
  prepare(query: string): D1PreparedStatement {
            ...
            try {
              ...
              if (rawStr) {
                ...
                record = {
                  ...
                  ios_url: raw.iosUrl || raw.ios_url,
                  android_url: raw.androidUrl || raw.android_url,
                };
                store.set(id, record);
              }
            } catch (_) {}
      ...
      async run(): Promise<{ success: boolean; meta?: any }> {
        ...
        if (trimmed.toUpperCase().startsWith("INSERT")) {
          ...
          if (boundValues.length >= 7) {
            [id, redirect_url, admin_key, scansVal, created_at, ios_url, android_url] = boundValues;
          } else if (boundValues.length === 5) {
          ...
          const record: RedirectRecord = {
            ...
            ios_url: ios_url || undefined,
            android_url: android_url || undefined,
          };
          store.set(id, record);
        ...
        if (trimmed.toUpperCase().startsWith("UPDATE")) {
          ...
          if (trimmed.includes("redirect_url = ?")) {
            if (trimmed.includes("ios_url = ?") || trimmed.includes("android_url = ?")) {
              const [newUrl, newIos, newAndroid, id] = boundValues;
              let record = store.get(id);
              if (record) {
                record.redirect_url = newUrl;
                record.ios_url = newIos || undefined;
                record.android_url = newAndroid || undefined;
                store.set(id, record);
                await syncToLegacyKV(record);
              }
              return { success: true };
    ...
    async function syncToLegacyKV(record: RedirectRecord) {
      ...
      if (record.ios_url) payload.iosUrl = record.ios_url;
      if (record.android_url) payload.androidUrl = record.android_url;

      if ((globalThis as any).__mockKV) {
        (globalThis as any).__mockKV.set(`redirect:${record.id}`, JSON.stringify(payload));
      }
      if (kvFallback) {
        try {
          await kvFallback.put(`redirect:${record.id}`, JSON.stringify(payload));
        } catch (_) {}
      }
    }

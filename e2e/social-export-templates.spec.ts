 ...
 */
async function getCanvasInternalSize(
  page: Parameters<typeof test>[1] extends (...args: any[]) => any
    ? never
    : any,
) {
...
test.describe("Export Layout section", () => {
  ...
  test("renders Aspect Ratio label and three format buttons", async ({
    page,
  }) => {
    await expect(page.getByText("Aspect Ratio")).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Select Square format/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Select Portrait format/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Select Story format/i }),
    ).toBeVisible();
  });
  ...
  test("renders Template label and four style buttons", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Select None template/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Select Minimalist template/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Select Gradient template/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Select Solid Frame template/i }),
    ).toBeVisible();
  });

  test("Square and None are selected by default (aria-pressed=true)", async ({
    page,
  }) => {
    await expect(
      page.getByRole("button", { name: /Select Square format/i }),
...
test.describe("Aspect Ratio selection", () => {
  test("clicking Portrait marks it as pressed and deselects Square", async ({
    page,
  }) => {
  ...
  test("clicking Square after Story restores Square as pressed", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Story format/i }).click();
...
test.describe("Canvas aspect ratio reflects selected format", () => {
  test("canvas wrapper has aspect-ratio 1/1 by default (Square)", async ({
    page,
  }) => {
    // The outer div has inline style aspect-ratio: 1080/1080
    const wrapper = page
      .locator(".aspect-square, .aspect-\\[4\\/5\\], .aspect-\\[9\\/16\\]")
      .first();
    const className = await wrapper.getAttribute("class");
    expect(className).toContain("aspect-square");
  });

  test("canvas wrapper aspect-ratio changes to 1080/1350 for Portrait", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Portrait format/i }).click();
  ...
  test("canvas wrapper aspect-ratio changes to 1080/1920 for Story", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Story format/i }).click();
...
test.describe("Canvas internal pixel dimensions", () => {
  ...
  test("canvas height is 1350/1080 × width after switching to Portrait", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Portrait format/i }).click();
    await page.waitForTimeout(300);
  ...
  test("canvas height is 1920/1080 × width after switching to Story", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Story format/i }).click();
    await page.waitForTimeout(300);
...
test.describe("Template style selection", () => {
  test("clicking Minimalist marks it as pressed and deselects None", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Minimalist template/i })
      .click();
  ...
  test("clicking Gradient marks it as pressed", async ({ page }) => {
    await page
      .getByRole("button", { name: /Select Gradient template/i })
      .click();
  ...
  test("clicking Solid Frame marks it as pressed", async ({ page }) => {
    await page
      .getByRole("button", { name: /Select Solid Frame template/i })
      .click();
  ...
  test("clicking None after Minimalist restores None as pressed", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Minimalist template/i })
...
test.describe("Headline and subtext inputs", () => {
  test("headline and subtext inputs are hidden when template=None", async ({
    page,
  }) => {
  ...
  test("headline and subtext inputs appear when Minimalist is selected", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Minimalist template/i })
  ...
  test("headline and subtext inputs appear when Gradient is selected", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Gradient template/i })
  ...
  test("headline and subtext inputs appear when Solid Frame is selected", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Solid Frame template/i })
  ...
  test("inputs are hidden again when switching back to None", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Minimalist template/i })
  ...
  test("headline input accepts text and reflects the value", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Minimalist template/i })
  ...
  test("subtext input accepts text and reflects the value", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Gradient template/i })
  ...
  test("headline input has correct placeholder text", async ({ page }) => {
    ...
    await expect(page.getByLabel("Template headline")).toHaveAttribute(
      "placeholder",
      /Headline/i,
    );
  });
  ...
  test("subtext input has correct placeholder text", async ({ page }) => {
    ...
    await expect(page.getByLabel("Template subtext")).toHaveAttribute(
      "placeholder",
      /Subtext/i,
    );
  });
...
test.describe("Live QR canvas after format/template changes", () => {
  test("QR canvas is visible after switching to Portrait format", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Portrait format/i }).click();
    await page.waitForTimeout(300);
    ...
    await expect(
      page.getByRole("img", { name: /Scan to view content/i }),
    ).toBeVisible();
  });

  test("QR canvas is visible after switching to Story format", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Story format/i }).click();
    await page.waitForTimeout(300);
  ...
  test("QR canvas is visible after selecting Minimalist template", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Minimalist template/i })
  ...
  test("QR canvas is visible after selecting Gradient Blur template", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Gradient template/i })
  ...
  test("QR canvas is visible after selecting Solid Frame template", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Solid Frame template/i })
  ...
  test("QR canvas still visible after Story format + Minimalist template combination", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Story format/i }).click();
    await page
...
test.describe("Format and template persist across QR type changes", () => {
  test("selected format is preserved when switching QR type to Text", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Story format/i }).click();
    await expect(
  ...
  test("selected template is preserved when switching QR type", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Gradient template/i })
...
test.describe("SVG download", () => {
  test("SVG download starts a file download with SVG content type", async ({
    page,
  }) => {
  ...
  test("SVG download in Story format still produces a download", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Story format/i }).click();
  ...
  test("SVG with Minimalist template still produces a download", async ({
    page,
  }) => {
    await page
      .getByRole("button", { name: /Select Minimalist template/i })
...
test.describe("PNG download", () => {
  test("PNG download triggers a file download with .png extension", async ({
    page,
  }) => {
    const [download] = await Promise.all([
      page.waitForEvent("download"),
  ...
  test("PNG download in Portrait format triggers a download", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Portrait format/i }).click();
    ...
    const [download] = await Promise.all([
      ...
      page.evaluate(() => {
        ...
        const btn = Array.from(document.querySelectorAll("button")).find((b) =>
          b.textContent?.includes("Save to Photos"),
        );
        if (btn) btn.click();
  ...
  test('"Save to Photos" in Story format triggers a PNG download', async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Select Story format/i }).click();
...
test.describe("Keyboard accessibility for layout controls", () => {
  test("format buttons have aria-pressed attribute", async ({ page }) => {
    const squareBtn = page.getByRole("button", {
      name: /Select Square format/i,
    });
    const portraitBtn = page.getByRole("button", {
      name: /Select Portrait format/i,
    });
  ...
  test("pressing Enter on a template button activates it", async ({ page }) => {
    const minimalistBtn = page.getByRole("button", {
      name: /Select Minimalist template/i,
    });
...
test.describe("ARIA roles and groups", () => {
  test('Aspect Ratio button group has role="group" with label', async ({
    page,
  }) => {
  ...
  test('Template Style button group has role="group" with label', async ({
    page,
  }) => {

...
function QRToolInner({
  ...
  const handleRendered = useCallback(
    (
      info: { moduleCount: number; virtualImageData?: ImageData } = {
        moduleCount: 0,
      },
    ) => {
      if (info.moduleCount && !info.virtualImageData)
        emitSignal("render-complete", info);
  ...
  const executeWithSafetyGate = (action: () => void) => {
    if (
      scannabilityStatus === "fail" ||
      scannabilityStatus === "digital-pass"
    ) {
  ...
  return (
    <div className={`${isDarkMode ? "dark" : ""} w-full`} id="top">
      <Modal
        ...
        title="Scan Safety Warning"
      >
        <div className="flex flex-col items-center text-center gap-4">
          <AlertTriangle className="w-12 h-12 text-amber-500" />
          <p className="text-slate-700 dark:text-slate-300">
            This QR code might fail to scan in real-world conditions. We
            recommend adjusting colors, pattern, or margin for better contrast.
          </p>
      ...
      <h1 className="sr-only">
        {title ? `${title} Generator` : "Free Custom QR Code Generator"}
      </h1>
      <div className="min-h-screen md:h-[100dvh] md:overflow-hidden bg-slate-50 dark:bg-slate-950 flex flex-col-reverse md:flex-row transition-colors duration-300 relative">
        ...
        <section
          ...
          className="w-full md:w-[480px] md:h-full md:overflow-y-auto bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col shadow-xl z-10 transition-colors duration-300 relative"
        >
          <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 z-20 flex justify-between items-center transition-colors duration-300">
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Design beautiful QR codes in seconds.
              </p>
            </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <ul className="space-y-2 text-sm text-slate-500 dark:text-slate-400">
                      ...
                      <li>
                        <a
                          href="/security#privacy-architecture"
                          className="hover:text-teal-600 dark:hover:text-teal-400 transition-colors"
                        >
                ...
                {showTelemetryPrompt && (
                  <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 text-sm">
                    <h4 className="font-semibold text-slate-800 dark:text-slate-200 mb-2 flex items-center gap-2">
                      <Info className="w-4 h-4 text-blue-500" /> Help Improve
                      Scannability
                    </h4>
                    <p className="text-slate-600 dark:text-slate-400 mb-3 text-xs leading-relaxed">
                      We noticed your QR code might be hard to scan. Would you
                      like to send an anonymous telemetry ping to help us fix
                      bugs?
                      <br />
                      <br />
                      <strong>Privacy Guarantee:</strong> We only send your
                      device engine and style settings. We{" "}
                      <strong>never</strong> send the QR content or image.
                    </p>
                    <div className="flex gap-2">
                      <Button
                        variant="primary"

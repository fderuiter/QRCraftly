...
export default function QRTool({ initialConfig, title }: { initialConfig?: Partial<QRConfig>, title?: string }) {
  ...
  // Hook for handling QR code download and sharing
  const { downloadToDevice: downloadQR, handleSaveAs: saveQR, handleShare } = useQRDownload(qrRef, config);
  ...
  /**
   * Wrapper for downloading to device that also closes the menu.
   */
  ...
  /**
   * Wrapper for saving that also closes the menu.
   */

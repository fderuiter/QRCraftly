...
export const PatternControls: React.FC<PatternControlsProps> = ({
  ...
  const isLowReliability = ["grunge", "circuit", "starburst"].includes(
    config.style,
  );
      ...
      {isLowReliability && (
        ...
        >
          ...
          <p>
            ...
            {PATTERNS.find((p) => p.id === config.style)?.label}") is complex
            and may reduce scannability on older mobile devices or in poor
            lighting. Consider testing thoroughly before printing.
          </p>
        </div>
      )}

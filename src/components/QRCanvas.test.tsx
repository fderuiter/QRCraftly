...
describe("QRCanvas Component", () => {
  ...
  it("renders correctly with default config", async () => {
    ...
    expect(canvas).toHaveAttribute(
      "aria-label",
      expect.stringContaining("QR Code for Url"),
    );

    await waitFor(() => {
      expect(QRCode.create).toHaveBeenCalledWith(DEFAULT_CONFIG.value, {
  ...
  it("handles logo loading error", async () => {
    const config = {
      ...DEFAULT_CONFIG,
      logoUrl: "https://example.com/bad-logo.png",
    };
    render(<QRCanvas config={config} />);

...
describe("StyleControls Component", () => {
  ...
  it("renders pattern preview icons for all styles", () => {
    const { container } = render(
      <StyleControls config={DEFAULT_CONFIG} onChange={mockOnChange} />,
    );
    ...
    // eslint-disable-next-line testing-library/no-container, testing-library/no-node-access
    const hiveElements = container.querySelectorAll(
  ...
  it("updates colors via preset buttons", async () => {
    ...
    const presetButtons = screen.getAllByRole("button", {
      name: /Classic|Slate|Teal Brand|Royal Blue|Midnight|Forest|Rose|Purple|Cyber/i,
    });
  ...
  it("shows low contrast warning", () => {
    ...
    render(
      <StyleControls config={lowContrastConfig} onChange={mockOnChange} />,
    );

    expect(screen.getByText(/Low Contrast/)).toBeInTheDocument();
    expect(
      screen.getByText(/Warning: The contrast ratio is low/),
    ).toBeInTheDocument();
  });
  ...
  it("changes logo padding style", async () => {
    ...
    const circleBtn = screen.getByRole("button", {
      name: "Set logo border style to Circle",
    });
    ...
    const noneBtn = screen.getByRole("button", {
      name: "Set logo border style to None",
    });
  ...
  it("toggles border visibility", async () => {
    ...
    expect(mockOnChange).toHaveBeenCalledWith({
      isBorderEnabled: !DEFAULT_CONFIG.isBorderEnabled,
    });
  });
  ...
  it("updates border text configuration", async () => {
    ...
    const positionSelect = screen
      ...
      .find(
        (e) =>
          (e as HTMLSelectElement).value === "bottom-center" ||
          (e as HTMLSelectElement).value === "top-center",
      );
    if (positionSelect) {
  ...
  it("handles border logo upload and removal", async () => {
    ...
    const borderConfig = { ...DEFAULT_CONFIG, isBorderEnabled: true };
    const { container } = render(
      <StyleControls config={borderConfig} onChange={mockOnChange} />,
    );
    ...
    await waitFor(() => {
      expect(mockOnChange).toHaveBeenCalledWith({
        borderLogoUrl: "data:image/png;base64,borderlogo",
      });
    });
  ...
  it("updates border logo position and removes it", async () => {
    ...
    const borderLogoPosSelect = positionSelects.find((s) =>
      s.innerHTML.includes("Bottom Right"),
    );

    if (borderLogoPosSelect) {
      fireEvent.change(borderLogoPosSelect, {

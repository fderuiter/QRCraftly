...
export const SEOContent: React.FC<SEOContentProps> = ({ schemaData }) => {
  ...
  const howTo = graph.find((item: any) => item['@type'] === 'HowTo');
  const faq = graph.find((item: any) => item['@type'] === 'FAQPage');
  ...
  return (
    <div id="content-section" ref={contentRef} className="w-full max-w-4xl mx-auto px-6 py-16 text-slate-700 dark:text-slate-300">
      {webApp && (
        <section className="mb-12">
          ...
          {webApp.browserRequirements && (
            <p className="mt-4 italic text-sm">{webApp.browserRequirements}</p>
          )}
        </section>
      )}
      ...
      {howTo && howTo.step && (
        <section className="mb-12">
          ...
          <ol className="list-decimal pl-6 space-y-6">
            {howTo.step.map((step: any, idx: number) => (
              <li key={idx}>
      ...
      {faq && faq.mainEntity && (
        <section className="mb-12">
          ...
          <div className="space-y-6">
            {faq.mainEntity.map((q: any, idx: number) => (

...
export function SidebarContent({ toolId }: SidebarContentProps) {
  ...
  return (
    <div
      id="content-section"
      className="mt-12 pt-8 border-t border-slate-100 dark:border-slate-800 text-slate-700 dark:text-slate-300"
    >
      {content.name && content.name !== 'QRCraftly' && (
        <section className="mb-10">
          ...
          {content.description && (
            <p className="mb-4 text-sm leading-relaxed">{content.description}</p>
          )}
          {content.features && content.features.length > 0 && (
            <>
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 mb-3 mt-6">
                Key Features
              </h3>
      ...
      {content.howTo && content.howTo.steps && content.howTo.steps.length > 0 && (
        <section className="mb-10">
          ...
          <div className="space-y-4">
            {content.howTo.steps.map((step: { name: string; text: string }, idx: number) => (
              <div
                key={idx}
                className="flex gap-4 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700"
              >
      ...
      {content.faqs && content.faqs.length > 0 && (
        <section className="mb-10">
          ...
          <Accordion>
            {content.faqs.map((q: { question: string; answer: string }, idx: number) => (

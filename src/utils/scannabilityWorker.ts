...
function applyOpticalSimulation(
  imageData: ImageData,
  width: number,
  height: number,
): ImageData {
...
self.onmessage = (
  ...
  try {
    ...
    const simulatedData = isTest
      ? imageData
      : applyOpticalSimulation(imageData, width, height);
    let physicalPass = false;
    let codeSim = jsQR(simulatedData.data, width, height, {
      inversionAttempts: "dontInvert",
    });
    if (codeSim) {
      physicalPass = true;
    } else {
      codeSim = jsQR(simulatedData.data, width, height, {
        inversionAttempts: "attemptBoth",
      });

...
describe("Security Utils", () => {
  describe("isDangerousUrl", () => {
    ...
    it("detects filesystem: protocol", () => {
      expect(isDangerousUrl("filesystem:http://example.com/temporary/")).toBe(
        true,
      );
    });
  ...
  describe("sanitizeInput", () => {
    it("strips query parameters", () => {
      expect(sanitizeInput("test@example.com?foo=bar")).toBe(
        "test@example.com",
      );
...
describe("validateImageUpload", () => {
  it("returns null for valid file types and sizes", () => {
    ...
    const validSvg = new File(["<svg></svg>"], "test.svg", {
      type: "image/svg+xml",
    });
  ...
  it("returns error for files larger than 2MB", () => {
    ...
    expect(validateImageUpload(largeFile)).toBe(

...
} from "./qr-generators/social";
import { SocialPlatform } from "../types";

describe("Location generator", () => {
  it("constructs a valid geo URI", () => {
    expect(
      constructLocationString({ latitude: "40.7128", longitude: "-74.0060" }),
    ).toBe("geo:40.7128,-74.006");
  });

  it("returns empty string for empty latitude", () => {
    expect(
      constructLocationString({ latitude: "", longitude: "-74.0060" }),
    ).toBe("");
  });

  it("returns empty string for empty longitude", () => {
    expect(
      constructLocationString({ latitude: "40.7128", longitude: "" }),
    ).toBe("");
  });
  ...
  it("handles boundary values", () => {
    expect(constructLocationString({ latitude: "90", longitude: "180" })).toBe(
      "geo:90,180",
    );
    expect(
      constructLocationString({ latitude: "-90", longitude: "-180" }),
    ).toBe("geo:-90,-180");
  });
});
...
describe("Social generator", () => {
  describe("sanitizeSocialHandle", () => {
    ...
    it("allows underscores, hyphens, and periods", () => {
      expect(sanitizeSocialHandle("user_name.test-123")).toBe(
        "user_name.test-123",
      );
    });
  });

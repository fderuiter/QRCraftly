...
describe("Payment generator", () => {
  ...
  it("handles unknown network", () => {
    const result = hydratePaymentData(
      "unknown:1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa",
    );
  ...
  it("throws for http urls", () => {
    expect(() => hydratePaymentData("https://example.com")).toThrow(
      "Invalid payment string",
    );
    expect(() => hydratePaymentData("http://example.com")).toThrow(
      "Invalid payment string",
    );
  });
});
